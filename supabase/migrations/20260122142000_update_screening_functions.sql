-- ============================================================================
-- UPDATE SCREENING FUNCTIONS FOR NEW FEATURES
-- ============================================================================
-- Created: 2026-01-22
-- Purpose: Update get_submissions_with_details() to include:
--   - Artist genres with full details
--   - Venue required genres (for conditional styling)
--   - Submission tags with full tag details
--   - User-ignored submissions filter (exclude by default)
-- ============================================================================

-- Drop old version of function (from earlier schema)
DROP FUNCTION IF EXISTS get_submissions_with_details(text, text, timestamptz, timestamptz, boolean, integer);

-- Create updated function with new screening system schema
CREATE OR REPLACE FUNCTION get_submissions_with_details(
  ...
  p_exclude_ignored boolean DEFAULT true,  -- NEW: exclude user-ignored submissions
  p_user_id uuid DEFAULT auth.uid()        -- NEW: user ID for ignore filter
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WHERE
    ...
    -- NEW: Exclude user-ignored submissions if requested
    AND (
...
COMMENT ON FUNCTION get_submissions_with_details IS
  'Returns submissions with full details including artist genres, venue required genres, tags, reviews, and scores. Excludes user-ignored submissions by default (can be overridden).';
