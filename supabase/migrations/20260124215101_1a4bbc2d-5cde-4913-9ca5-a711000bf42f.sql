-- =====================================================
-- Secure get_all_users() function - Restrict to admins/developers only
-- =====================================================
-- This migration adds authorization checks to the get_all_users() function
-- to prevent unauthorized access to auth.users data via the users_complete view.
...
-- Step 3: Recreate the function with authorization check
...
AS $$
BEGIN
  -- Authorization check: Only admins and developers can access user data
  IF NOT (
...
-- Step 5: Re-grant permissions (view still accessible, but function will throw error for non-admins)
...
SECURITY: Restricted to admin and developer roles only. 
...
SECURITY: Only accessible by admin and developer roles.';