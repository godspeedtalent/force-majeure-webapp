...
export class TicketPDFGenerator {
  ...
  static async generateMultipleTickets(
    ...
    try {
      ...
      const firstTicketBase64 = await this.generateSingleTicket(
        tickets[0],
        options
      );
      const firstTicketData = `data:application/pdf;base64,${firstTicketBase64}`;
      ...
      for (let i = 1; i < tickets.length; i++) {
        ...
        const ticketPdf = await this.generateSingleTicket(tickets[i], options);
        const ticketData = `data:application/pdf;base64,${ticketPdf}`;
