 ...
 */
export const TopographicBackgroundDebug = ({
  opacity = 0.1,
  className,
  showLabels = true,
}: TopographicBackgroundDebugProps) => {
  ...
  return (
    <div
      className={cn('absolute inset-0 pointer-events-none overflow-auto', className)}
      style={{ opacity, backgroundColor: '#000' }}
    >
      <div className='relative' style={{ minHeight: `${TILE_SIZE * 4}px` }}>
        {configs.map((config, configIndex) => (
          <div key={configIndex}>
            {/* Config label */}
            {showLabels && (
              <div
                className='absolute text-fm-gold font-bold text-xl bg-black/80 px-4 py-2 border border-fm-gold'
                style={{
                  left: '20px',
            ...
            {/* Render tiles for this configuration */}
            {config.tiles.map((tile, tileIndex) => {
              ...
              return (
                <div key={tileIndex}>
                  ...
                  {showLabels && (
                    <div
                      className='absolute text-white text-sm bg-black/60 px-2 py-1 font-mono'
                      style={{
            ...
            {/* Draw vertical lines at tile boundaries */}
            <div
