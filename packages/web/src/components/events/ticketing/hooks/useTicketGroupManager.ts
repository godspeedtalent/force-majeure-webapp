...
import { TicketGroup, TicketTier } from '../types';
...
interface UseTicketGroupManagerProps {
  groups: TicketGroup[];
 ...
 */
export function useTicketGroupManager({ groups, onChange }: UseTicketGroupManagerProps) {
  const [activeView, setActiveView] = useState<'overview' | string>('overview');

  // Calculate totals for overview
  const totalTickets = groups.reduce(
    (sum, group) =>
      sum + group.tiers.reduce((tierSum, tier) => tierSum + tier.total_tickets, 0),
    0
  );
