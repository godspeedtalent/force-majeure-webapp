 ...
 */
export const assertSessionStatus = async (
  userSessionId: string,
  expectedStatus: 'active' | 'waiting' | 'completed'
): Promise<void> => {
  ...
  if (session.status !== expectedStatus) {
    throw new Error(
      `Expected session status to be '${expectedStatus}' but got '${session.status}'`
    );
  }
 ...
 */
export const assertSessionNotExists = async (
  userSessionId: string
): Promise<void> => {
  const session = await getSession(userSessionId);
  ...
  if (session) {
    throw new Error(
      `Expected session not to exist, but found session with status '${session.status}'`
    );
  }
 ...
 */
export const assertQueuePosition = async (
  userSessionId: string,
  ...
  if (session.status !== 'waiting') {
    throw new Error(
      `Cannot check queue position for session with status '${session.status}'. Must be 'waiting'.`
    );
  }
  ...
  if (actualPosition !== expectedPosition) {
    throw new Error(
      `Expected queue position ${expectedPosition} but got ${actualPosition}`
    );
  }
 ...
 */
export const assertSessionCount = async (
  eventId: string,
  ...
  if (actualCount !== expectedCount) {
    throw new Error(
      `Expected ${expectedCount} sessions with status '${status}' but got ${actualCount}`
    );
  }
 ...
 */
export const assertInventoryDecremented = async (
  tierId: string,
  ...
  if (originalInventory !== undefined) {
    ...
    if (tier.available_inventory !== expectedInventory) {
      throw new Error(
        `Expected inventory to be ${expectedInventory} but got ${tier.available_inventory}`
      );
    }
  }
 ...
 */
export const assertOrderCreated = async (
  orderId: string,
  ...
  if (expectedUserId && order.user_id !== expectedUserId) {
    throw new Error(
      `Expected order user_id to be '${expectedUserId}' but got '${order.user_id}'`
    );
  }

  if (expectedEventId && order.event_id !== expectedEventId) {
    throw new Error(
      `Expected order event_id to be '${expectedEventId}' but got '${order.event_id}'`
    );
  }
 ...
 */
export const assertOrderItemsCreated = async (
  orderId: string,
  ...
  if (actualItemCount !== expectedItemCount) {
    throw new Error(
      `Expected ${expectedItemCount} order items but got ${actualItemCount}`
    );
  }
 ...
 */
export const assertCartContains = (
  cart: any[],
  ...
  if (expectedQuantity !== undefined && item.quantity !== expectedQuantity) {
    throw new Error(
      `Expected item quantity to be ${expectedQuantity} but got ${item.quantity}`
    );
  }
...
export const assertCartTotal = (
  ...
  if (Math.abs(actualTotal - expectedTotal) > tolerance) {
    throw new Error(
      `Expected cart total to be ${expectedTotal} but got ${actualTotal}`
    );
  }
 ...
 */
export const assertLocalStorageContains = (key: string): void => {
 ...
 */
export const assertLocalStorageNotContains = (key: string): void => {
  const value = localStorage.getItem(key);
...
export const assertInRange = (
  ...
  if (value < min || value > max) {
    throw new Error(
      `${label} (${value}) is not within range [${min}, ${max}]`
    );
  }
