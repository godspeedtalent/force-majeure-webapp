...
describe('calculateEstimatedWaitTime', () => {
  ...
  describe('when slots are available', () => {
    it('returns 0 wait time if user can enter immediately', () => {
      ...
      expect(
        calculateEstimatedWaitTime(1, 5, maxConcurrent, avgSessionMinutes)
      ).toBe(0);
      expect(
        calculateEstimatedWaitTime(5, 5, maxConcurrent, avgSessionMinutes)
      ).toBe(0);
    });
    ...
    it('calculates wait time for positions beyond available slots', () => {
      ...
      expect(
        calculateEstimatedWaitTime(6, 5, maxConcurrent, avgSessionMinutes)
      ).toBe(5); // 1 group * 5 min

      // Position 10 needs to wait for 5 users (rounds up to 1 session)
      expect(
        calculateEstimatedWaitTime(10, 5, maxConcurrent, avgSessionMinutes)
      ).toBe(5);

      // Position 11 needs to wait for 6 users (rounds up to 1 session)
      expect(
        calculateEstimatedWaitTime(11, 5, maxConcurrent, avgSessionMinutes)
      ).toBe(5);

      // Position 16 needs to wait for 11 users (rounds up to 2 sessions)
      expect(
        calculateEstimatedWaitTime(16, 5, maxConcurrent, avgSessionMinutes)
      ).toBe(10); // 2 groups * 5 min
    });
  });
  ...
  describe('when at max capacity', () => {
    it('calculates wait time based on full batches', () => {
      ...
      expect(
        calculateEstimatedWaitTime(1, 10, maxConcurrent, avgSessionMinutes)
      ).toBe(5);

      // Position 10 waits for 1 session
      expect(
        calculateEstimatedWaitTime(10, 10, maxConcurrent, avgSessionMinutes)
      ).toBe(5);

      // Position 11 waits for 2 sessions
      expect(
        calculateEstimatedWaitTime(11, 10, maxConcurrent, avgSessionMinutes)
      ).toBe(10);

      // Position 20 waits for 2 sessions
      expect(
        calculateEstimatedWaitTime(20, 10, maxConcurrent, avgSessionMinutes)
      ).toBe(10);

      // Position 21 waits for 3 sessions
      expect(
        calculateEstimatedWaitTime(21, 10, maxConcurrent, avgSessionMinutes)
      ).toBe(15);
    });
  });
  ...
  describe('edge cases', () => {
    it('handles position 0 correctly', () => {
      expect(
        calculateEstimatedWaitTime(0, 5, maxConcurrent, avgSessionMinutes)
      ).toBe(0);
    });
    ...
    it('handles no active users', () => {
      ...
      expect(
        calculateEstimatedWaitTime(1, 0, maxConcurrent, avgSessionMinutes)
      ).toBe(0);
      expect(
        calculateEstimatedWaitTime(10, 0, maxConcurrent, avgSessionMinutes)
      ).toBe(0);
      expect(
        calculateEstimatedWaitTime(11, 0, maxConcurrent, avgSessionMinutes)
      ).toBe(5);
    });
