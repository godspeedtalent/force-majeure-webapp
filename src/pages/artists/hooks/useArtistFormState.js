...
import { createInitialArtistFormState, } from '../types/artistForm';
/**
 * useArtistFormState
 *
 ...
 * ```
 */
export function useArtistFormState(options = {}) {
    const { onAutoSave } = options;
    const [state, setState] = useState(createInitialArtistFormState);
    // Build metadata object for saving
    const buildMetadata = useCallback((currentState) => {
        const socialLinks = {};
        if (currentState.social.instagram)
            socialLinks.instagram = currentState.social.instagram;
        if (currentState.social.twitter)
            socialLinks.twitter = currentState.social.twitter;
        if (currentState.social.facebook)
            socialLinks.facebook = currentState.social.facebook;
        if (currentState.social.tiktok)
            socialLinks.tiktok = currentState.social.tiktok;
        if (currentState.social.youtube)
            socialLinks.youtube = currentState.social.youtube;
        return {
    ...
    const buildSaveData = useCallback((currentState) => ({
    ...
    const initializeFromArtist = useCallback((artist) => {
        if (!artist)
            return;
    ...
    // Update genres
    const updateGenres = useCallback((genres) => {
        setState(prev => ({
            ...prev,
            genres,
        }));
    ...
    const updateTrack = useCallback((updatedTrack) => {
        setState(prev => {
            const newState = {
                ...
                music: {
                    ...prev.music,
                    tracks: prev.music.tracks.map(t => t.id === updatedTrack.id ? updatedTrack : t),
                    editingTrack: null,
                },
            };
            triggerAutoSave(newState);
    ...
    const incrementTrackClick = useCallback((trackId) => {
        setState(prev => {
            const newState = {
                ...prev,
                music: {
                    ...prev.music,
                    tracks: prev.music.tracks.map(t => t.id === trackId
    ...
    const setEditingTrack = useCallback((track) => {
        setState(prev => ({
            ...prev,
