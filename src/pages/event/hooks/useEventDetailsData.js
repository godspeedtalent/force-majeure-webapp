...
export function useEventDetailsData(event) {
    ...
    const callTimeLineup = useMemo(() => {
        ...
        return lineup.map((artist, index) => {
            const callDate = hasValidBase && index >= 0
                ? new Date(baseDate.getTime() + index * CALL_TIME_INTERVAL_MINUTES * 60000)
                : null;
            const callTimeLabel = callDate !== null
                ? formatTimeDisplay(callDate.toLocaleTimeString('en-US', {
                    hour: 'numeric',
                    minute: '2-digit',
                }))
                : 'TBD';
