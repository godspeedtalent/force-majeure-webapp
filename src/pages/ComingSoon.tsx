...
export default function ComingSoon() {
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center relative overflow-hidden">
      ...
      {/* Content */}
      <div className={`relative z-10 text-center px-6 max-w-2xl mx-auto transition-opacity duration-500 ${fontsLoaded ? 'opacity-100' : 'opacity-0'}`}>
        {/* Logo */}
        ...
        {/* Main message */}
        <h1 className={`font-display text-4xl md:text-6xl mb-6 ${fontsLoaded ? 'animate-slide-down-in' : ''}`}>
          The Force Majeure Hub
          <br />
          <span className="text-fm-gold">Coming Soon</span>
        </h1>

        <p className={`text-lg md:text-l text-muted-foreground mb-12 ${fontsLoaded ? 'animate-fade-in' : ''}`} 
        ...
        {/* Social links */}
        <div className={`flex items-center justify-center gap-6 ${fontsLoaded ? 'animate-fade-in' : 'opacity-0'}`} style={{ animationDelay: fontsLoaded ? "0.4s" : "0s" }}>
          <a
