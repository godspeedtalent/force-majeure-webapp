...
const simulatePurchase = async (userId, eventId, eventTitle, userData, logger) => {
    ...
    try {
        ...
        const duration = Date.now() - startTime;
        logger.error(`${logPrefix} ❌ PURCHASE FAILED after ${duration}ms: ${error.message}`);
        throw error;
    }
};
      ...
      `.trim(),
            ...
            execute: async () => {
                ...
                const ticketsSold = results
                    ...
                    .reduce((sum, r) => sum + (r.value?.tickets || 0), 0);
                logger.info('='.repeat(80));
                logger.info(`📊 TEST RESULTS`);
                logger.info(`✅ Successful: ${successful}/${userCount} (${((successful / userCount) * 100).toFixed(1)}%)`);
                logger.info(`❌ Failed: ${failed}/${userCount} (${((failed / userCount) * 100).toFixed(1)}%)`);
...
export default function CheckoutFlowTests() {
    return (_jsx(TestSuiteRunner, { suite: performanceTestSuite, icon: ShoppingCart, options: {
            maxConcurrency: 7,
            timeout: 300000,
            retries: 0,
            stopOnError: false,
