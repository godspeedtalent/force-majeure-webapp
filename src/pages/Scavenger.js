...
import { useAllClaims, useAutoScroll, useClaimReward, useLocationClaimCount, useScavengerLocations, useScavengerNavigation, useUserClaims, } from '@/features/scavenger/hooks/useScavenger';
...
export default function Scavenger() {
    ...
    // Use custom hooks
    const { locationId, debugMode: _debugMode, showInvalidToken, navigate, } = useScavengerNavigation();
    ...
    const totalUndiscoveredCheckpoints = locations?.filter(location => !claimedLocationIds.has(location.id))
        .length || 0;
    // Count locations with 1 or fewer total claims
    const lowClaimLocationsCount = (() => {
        if (!locations || !allClaims)
            return undefined;
    ...
    if (authLoading || locationsLoading) {
        return (_jsx("div", { className: 'min-h-screen flex items-center justify-center bg-background', children: _jsx(FmCommonLoadingState, {}) }));
    }
    // Handle invalid token state (from error parameter)
    if (showInvalidToken || !locationId) {
        return (_jsx(ScavengerSplitLayout, { showShoppingCart: !isFeatureEnabled(FEATURE_FLAGS.COMING_SOON_MODE), children: _jsx(InvalidTokenView, {}) }));
    }
    ...
    if (locationId) {
        ...
        const alreadyClaimed = userClaims?.some(claim => claim.location_id === locationId);
        ...
        if (alreadyClaimed || (hasAnyClaim && user)) {
            return (_jsx(ScavengerSplitLayout, { showShoppingCart: !isFeatureEnabled(FEATURE_FLAGS.COMING_SOON_MODE), children: _jsx(UnauthenticatedWizard, { locationName: location?.name, onLoginSuccess: async () => {
                    ...
                    }, userFullName: profile?.full_name ?? undefined, isAuthenticated: true, hasAlreadyClaimed: true, claimCount: claimCount, lowClaimLocationsCount: lowClaimLocationsCount }) }));
        }
        ...
        if (!user) {
            ...
            return (_jsx(ScavengerSplitLayout, { showShoppingCart: !isFeatureEnabled(FEATURE_FLAGS.COMING_SOON_MODE), children: _jsx(UnauthenticatedWizard, { locationName: location?.name, onLoginSuccess: async () => {
                    ...
                    }, claimCount: claimCount, lowClaimLocationsCount: lowClaimLocationsCount }) }));
        }
        ...
        return (_jsx(ScavengerSplitLayout, { showShoppingCart: !isFeatureEnabled(FEATURE_FLAGS.COMING_SOON_MODE), children: _jsx(UnauthenticatedWizard, { locationName: location?.name, onLoginSuccess: async () => {
                ...
                }, userFullName: profile?.full_name ?? undefined, isAuthenticated: true, hasAlreadyClaimed: false, isClaimLoading: claimMutation.isPending, claimCount: claimCount, lowClaimLocationsCount: lowClaimLocationsCount }) }));
    }
    ...
    if (user && !locationId) {
        ...
        const hasAnyClaim = userClaims && userClaims.length > 0;
        if (hasAnyClaim) {
            return (_jsx(ScavengerSplitLayout, { showShoppingCart: !isFeatureEnabled(FEATURE_FLAGS.COMING_SOON_MODE), children: _jsx(UnauthenticatedWizard, { locationName: undefined, onLoginSuccess: async () => {
                        await queryClient.invalidateQueries({
                            queryKey: ['user-claims'],
                        });
                        await queryClient.invalidateQueries({ queryKey: ['all-claims'] });
                    ...
                    }, userFullName: profile?.full_name ?? undefined, isAuthenticated: true, hasAlreadyClaimed: true }) }));
        }
        return (_jsx(ScavengerSplitLayout, { showShoppingCart: !isFeatureEnabled(FEATURE_FLAGS.COMING_SOON_MODE), children: _jsx(AuthenticatedUserView, { displayName: profile?.display_name ?? undefined, totalUndiscoveredCheckpoints: totalUndiscoveredCheckpoints, isLoading: locationsLoading }) }));
    }
    // Show wizard if not authenticated (no user)
    if (!user) {
        return (_jsx(ScavengerSplitLayout, { showShoppingCart: !isFeatureEnabled(FEATURE_FLAGS.COMING_SOON_MODE), children: _jsx(UnauthenticatedWizard, { onLoginSuccess: async () => {
                    await queryClient.invalidateQueries({ queryKey: ['user-claims'] });
                    await queryClient.invalidateQueries({ queryKey: ['all-claims'] });
                    await queryClient.invalidateQueries({
                        queryKey: ['scavenger-locations'],
    ...
    return (_jsx(ScavengerFullLayout, { showShoppingCart: !isFeatureEnabled(FEATURE_FLAGS.COMING_SOON_MODE), locations: locations, userClaims: userClaims }));
}
