...
import { createMockEvent, TEST_PREFIXES, } from '../utils/mockData';
import { cleanupEventTestData, insertMockEvent, generateTestSessionId, waitFor, simulateDelay, countSessionsByStatus, } from '../utils/testHelpers';
import { assertSessionStatus, assertQueuePosition, assertSessionCount, } from '../utils/assertions';
export const queueTestSuite = {
    id: 'queue-tests',
    name: 'Queue Management Tests',
    description: 'Test the ticketing queue system including entry, position tracking, promotion, max capacity, and session cleanup',
    category: 'Queue',
    beforeAll: async () => {
    ...
    testCases: [
        {
            ...
            description: 'Test that a single user joining an empty queue gets immediate access (active status)',
        ...
        },
        {
            ...
            description: 'Test that when the first user exits, the second user is promoted from waiting to active',
        ...
        },
        {
            ...
            description: 'Test that when max concurrent users (50) is reached, the 51st user must wait',
        ...
        },
        {
            ...
            description: 'Test that queue position is recalculated correctly when users ahead exit',
        ...
        },
        {
            ...
            description: 'Test that sessions older than 30 minutes are automatically cleaned up',
        ...
        },
        {
            ...
            description: 'Test that 100 users joining simultaneously get correct position assignments',
            ...
            execute: async () => {
                ...
                try {
                    ...
                    for (let i = 0; i < NUM_USERS; i++) {
                        ...
                        insertPromises.push(supabase.from('ticketing_sessions').insert([insertData]));
