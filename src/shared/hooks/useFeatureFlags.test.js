...
import { isDevelopment, getEnvironmentOverride } from '@/shared';
...
describe('useFeatureFlags', () => {
    ...
    it('should apply environment overrides in development', async () => {
        ...
        vi.mocked(getEnvironmentOverride).mockImplementation((flagKey) => {
            if (flagKey === 'global_search')
                return true;
            return null;
        });
        vi.mocked(environmentService.getCurrentEnvironment).mockResolvedValue({
...
describe('useFeatureFlagHelpers', () => {
    ...
    describe('isAnyFeatureEnabled', () => {
        it('should return true when at least one flag is enabled', async () => {
            ...
            expect(result.current.isAnyFeatureEnabled(FEATURE_FLAGS.GLOBAL_SEARCH, FEATURE_FLAGS.SCAVENGER_HUNT)).toBe(true);
        });
        it('should return false when no flags are enabled', async () => {
            ...
            expect(result.current.isAnyFeatureEnabled(FEATURE_FLAGS.GLOBAL_SEARCH, FEATURE_FLAGS.SCAVENGER_HUNT)).toBe(false);
        });
    });
    describe('areAllFeaturesEnabled', () => {
        it('should return true when all flags are enabled', async () => {
            ...
            expect(result.current.areAllFeaturesEnabled(FEATURE_FLAGS.GLOBAL_SEARCH, FEATURE_FLAGS.SCAVENGER_HUNT)).toBe(true);
        });
        it('should return false when one flag is disabled', async () => {
            ...
            expect(result.current.areAllFeaturesEnabled(FEATURE_FLAGS.GLOBAL_SEARCH, FEATURE_FLAGS.SCAVENGER_HUNT)).toBe(false);
        });
    });
    ...
    describe('isLoading', () => {
        it('should return true while loading', () => {
            vi.mocked(environmentService.getCurrentEnvironment).mockImplementation(() => new Promise(() => { }) // Never resolves
            );
            const { result } = renderHook(() => useFeatureFlagHelpers(), { wrapper: createWrapper() });
