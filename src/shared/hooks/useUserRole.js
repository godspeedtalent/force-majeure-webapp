...
export const useUserPermissions = () => {
    ...
    const hasPermission = (permission) => {
        ...
        if (isAdmin())
            return true;
        return effectiveRoles.some(role => role.permission_names.includes(PERMISSIONS.ALL) ||
    ...
    const getRoles = () => {
        const effectiveRoles = getEffectiveRoles();
        return effectiveRoles.map(role => role.role_name);
    };
    /**
     ...
     */
    const isActuallyDeveloperOrAdmin = () => {
        if (!roles)
            return false;
        return roles.some(role => role.role_name === 'admin' || role.role_name === 'developer');
    };
    return {
        isAdmin,
        hasPermission,
