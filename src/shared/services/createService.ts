...
export interface ServiceConfig<TTable extends TableName = TableName> {
  /** Supabase table name */
  tableName: TTable;
...
const getTable = (table: TableName) => supabase.from(table) as any;

/**
 * Creates a base service with standard CRUD operations
 */
export function createService<T, CreateData, TTable extends TableName = TableName>(
  config: ServiceConfig<TTable>
...
// Export the TableName type for use elsewhere
