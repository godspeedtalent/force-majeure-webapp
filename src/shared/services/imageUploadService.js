...
export const imageUploadService = {
     ...
     */
    async uploadImage({ file, bucket = 'event-images', path, eventId, isPrimary = false, compressionOptions, }) {
        // Validate file type
        const validTypes = [
            'image/jpeg',
            'image/jpg',
        ...
        if (processedFile.size > maxSize) {
            throw new Error(`File size exceeds 5MB limit even after compression. Current size: ${(processedFile.size / 1024 / 1024).toFixed(2)}MB`);
        }
     ...
     */
    async deleteImage(imageId, bucket = 'event-images') {
        // Get image metadata
     ...
     */
    async setPrimaryImage(imageId, eventId) {
     ...
     */
    getPublicUrl(storagePath, bucket = 'event-images') {
