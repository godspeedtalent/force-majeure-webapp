...
export const compressImage = async (file, options = {}) => {
    const { maxWidth = 1920, maxHeight = 1920, maxSizeBytes = 5 * 1024 * 1024, // 5MB
    quality = 0.85, outputFormat, forceResize = false, } = options;
        ...
        img.onload = () => {
            try {
                ...
                canvas.toBlob(blob => {
                    ...
                    const compressedFile = new File([blob], file.name.replace(/\.[^.]+$/, mimeType === 'image/png' ? '.png' : mimeType === 'image/webp' ? '.webp' : '.jpg'), { type: mimeType });
