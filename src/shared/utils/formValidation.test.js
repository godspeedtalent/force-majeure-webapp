...
import { z } from 'zod';
import { stringRequired, stringOptional, emailField, passwordField, urlField, urlOptional, phoneField, dateField, dateOptional, futureDateField, pastDateField, positiveNumber, nonNegativeNumber, integerField, priceField, sanitizeInput, encodeForUrl, createFileValidation, contactFormSchema, eventFormSchema, profileFormSchema, passwordConfirmation, prepareFormData, } from './formValidation';
describe('stringRequired', () => {
    ...
    it('rejects string that exceeds max length', () => {
        ...
        expect(() => schema.parse('Too long name')).toThrow('Name must be less than 5 characters');
    });
...
describe('stringOptional', () => {
    ...
    it('rejects string exceeding max length', () => {
        ...
        expect(() => schema.parse('This is too long')).toThrow('Must be less than 10 characters');
    });
});
describe('emailField', () => {
    ...
    it('validates email with subdomains', () => {
        expect(emailField.parse('user@mail.example.com')).toBe('user@mail.example.com');
    });
});
describe('passwordField', () => {
    ...
    it('rejects password without uppercase letter', () => {
        expect(() => passwordField.parse('password123')).toThrow('one uppercase letter');
    });
    it('rejects password without lowercase letter', () => {
        expect(() => passwordField.parse('PASSWORD123')).toThrow('one lowercase letter');
    });
    ...
    it('rejects password longer than 128 characters', () => {
        ...
        expect(() => passwordField.parse(longPassword + 'X')).toThrow('less than 128 characters');
    });
});
describe('urlField', () => {
...
describe('urlOptional', () => {
    it('accepts valid URL', () => {
        expect(urlOptional.parse('https://example.com')).toBe('https://example.com');
    });
    it('accepts empty string', () => {
...
describe('phoneField', () => {
    ...
    it('rejects invalid phone format', () => {
        expect(() => phoneField.parse('abc-def-ghij')).toThrow('Invalid phone number');
    });
    it('rejects phone longer than 20 characters', () => {
        expect(() => phoneField.parse('1'.repeat(21))).toThrow('less than 20 characters');
    });
});
describe('dateField', () => {
...
describe('futureDateField', () => {
    ...
    it('rejects past date', () => {
        ...
        expect(() => futureDateField.parse(pastDate)).toThrow('Date must be in the future');
    });
    it('rejects current date', () => {
        const now = new Date();
        expect(() => futureDateField.parse(now)).toThrow('Date must be in the future');
    });
});
describe('pastDateField', () => {
    ...
    it('rejects future date', () => {
        ...
        expect(() => pastDateField.parse(futureDate)).toThrow('Date must be in the past');
    });
    it('rejects current date', () => {
        const now = new Date();
...
describe('nonNegativeNumber', () => {
    ...
    it('rejects negative number', () => {
        expect(() => nonNegativeNumber.parse(-5)).toThrow('Must be a non-negative number');
    });
});
...
describe('priceField', () => {
    ...
    it('rejects decimal price', () => {
        expect(() => priceField.parse(10.5)).toThrow('Price must be in cents (whole number)');
    });
...
describe('passwordConfirmation', () => {
    it('validates matching passwords', () => {
        const schema = passwordConfirmation(z.object({ password: z.string() }));
        const data = {
