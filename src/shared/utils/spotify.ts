...
export class SpotifyService {
  ...
  async getValidAccessToken(): Promise<string | null> {
    await logApi({
      ...
      method: 'POST',
      message: 'getValidAccessToken start',
    });
    ...
    if (
      ...
      await logApi({
        endpoint: 'spotify-auth',
        method: 'CACHE',
        message: 'Using cached Spotify token',
      });
  ...
  private async getClientCredentialsToken(): Promise<string | null> {
    try {
      await logApi({
        endpoint: 'spotify-auth',
        method: 'POST',
        message: 'Requesting client credentials token',
      });
      ...
      await logApi({
        endpoint: 'spotify-auth',
        method: 'POST',
        message: 'Received client credentials token',
  ...
  async searchTracks(query: string, limit: number = 20): Promise<any> {
    ...
    await logApi({
      endpoint: 'https://api.spotify.com/v1/search',
      method: 'GET',
      message: 'searchTracks success',
      details: { items: json?.tracks?.items?.length ?? 0 },
    });
    return json;
  }
  ...
  async getTrack(trackId: string): Promise<any> {
    ...
    await logApi({
      endpoint: `https://api.spotify.com/v1/tracks/${trackId}`,
      method: 'GET',
      message: 'getTrack success',
      details: { has_preview: !!json?.preview_url },
    });
    return json;
  }
  ...
  parseTrackIdFromLink(linkOrId: string): string | null {
    ...
    if (
      /\.mp3($|\?)/i.test(linkOrId) ||
      /p\.scdn\.co\/mp3-preview\//.test(linkOrId)
    ) {
      return null;
    }
  ...
  async getPreviewUrl(linkOrId: string): Promise<string | null> {
    await logApi({
      endpoint: 'spotify:getPreviewUrl',
      method: 'LOCAL',
      message: 'getPreviewUrl start',
    });
    ...
    if (
      ...
      await logApi({
        endpoint: 'spotify:getPreviewUrl',
        method: 'LOCAL',
        message: 'Input already preview URL',
      });
      return linkOrId;
    }
    ...
    await logApi({
      endpoint: 'spotify:getPreviewUrl',
      method: 'LOCAL',
      message: 'Resolved preview',
      details: { has_preview: !!preview },
    });
    return preview;
  }
   ...
   */
  async getAlbumArtUrl(
    linkOrId: string,
    size: 'small' | 'medium' | 'large' = 'medium'
  ): Promise<string | null> {
    await logApi({
      endpoint: 'spotify:getAlbumArtUrl',
      method: 'LOCAL',
      message: 'getAlbumArtUrl start',
      details: { size },
    });
    // If it's already an absolute image URL, return as-is
    if (
      /^https?:\/\//i.test(linkOrId) &&
      /\.(png|jpe?g|webp|gif)(\?.*)?$/i.test(linkOrId)
    ) {
      await logApi({
        endpoint: 'spotify:getAlbumArtUrl',
        method: 'LOCAL',
        message: 'Input already image URL',
      });
      return linkOrId;
    }
    ...
    const images:
      ...
      | undefined = track?.album?.images;
    if (!images || images.length === 0) return null;
    if (size === 'large')
      return images[0]?.url || images[1]?.url || images[2]?.url || null;
    if (size === 'small')
