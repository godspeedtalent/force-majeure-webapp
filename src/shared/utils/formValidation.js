 ...
 */
// String validations
export const stringRequired = (fieldName, maxLength = 255) => z
    .string()
    .trim()
    .min(1, `${fieldName} is required`)
    .max(maxLength, `${fieldName} must be less than ${maxLength} characters`);
export const stringOptional = (maxLength = 255) => z
    .string()
    .trim()
...
export const encodeForUrl = (input) => {
    return encodeURIComponent(input.trim());
};
// Validate file upload
export const createFileValidation = (maxSizeMB, allowedTypes) => {
    return z
        .instanceof(File)
