...
export function FmCommonSearchDropdown({ onChange, onSearch, onGetRecentOptions, onCreateNew, placeholder = 'Search...', createNewLabel = 'Create New', selectedLabel, disabled = false, typeIcon, typeTooltip, }) {
    ...
    return (_jsxs(Popover, { open: open, onOpenChange: setOpen, children: [_jsx(PopoverTrigger, { asChild: true, disabled: disabled, children: triggerButton }), _jsxs(PopoverContent, { className: 'w-[400px] p-0 bg-black/90 backdrop-blur-md border border-white/20', align: 'start', onOpenAutoFocus: (e) => {
                ...
                }, children: [_jsxs("div", { className: 'p-2 border-b border-white/10 relative', children: [_jsx(Input, { ref: inputRef, placeholder: placeholder, value: query, onChange: e => setQuery(e.target.value), onFocus: () => setIsFocused(true), onBlur: () => setIsFocused(false), onClick: e => {
                                    e.stopPropagation();
                                    inputRef.current?.focus();
                                }, className: 'bg-black/40 border-white/20 text-white placeholder:text-white/50 pr-8', autoFocus: true }), showClearButton && (_jsx("button", { onClick: handleClear, onMouseDown: e => e.preventDefault(), className: 'absolute right-4 top-1/2 -translate-y-1/2 text-white/50 hover:text-fm-gold transition-colors', "aria-label": 'Clear search', children: _jsx(X, { className: 'h-4 w-4' }) }))] }), _jsx("div", { className: 'max-h-[300px] overflow-y-auto', children: loading ? (_jsxs("div", { className: 'p-4 flex flex-col items-center gap-2', children: [_jsx(FmCommonLoadingSpinner, { size: 'md' }), _jsx("span", { className: 'text-white/50 text-sm', children: "Searching..." })] })) : query.length === 0 && recentOptions.length > 0 ? (_jsxs("div", { children: [_jsx("div", { className: 'px-3 py-2 text-xs text-white/50 font-semibold uppercase', children: "Recent" }), recentOptions.map(option => (_jsxs("button", { onClick: () => handleSelect(option), className: 'w-full flex items-center gap-3 px-3 py-2 hover:bg-white/10 transition-colors text-left', children: [option.icon, _jsx("span", { className: 'text-white font-light', children: option.label })] }, option.id)))] })) : options.length === 0 && query.length > 0 ? (_jsx("div", { className: 'p-4 text-center text-white/50', children: "No results found" })) : (options.map(option => (_jsxs("button", { onClick: () => handleSelect(option), className: 'w-full flex items-center gap-3 px-3 py-2 hover:bg-white/10 transition-colors text-left', children: [option.icon, _jsx("span", { className: 'text-white font-light', children: option.label })] }, option.id)))) }), onCreateNew && (_jsx("div", { className: 'border-t border-fm-gold', children: _jsx("button", { onClick: handleCreateNew, className: 'w-full flex items-center gap-2 px-3 py-2 hover:bg-fm-gold/10 transition-colors text-fm-gold font-medium text-sm', children: _jsx("span", { children: createNewLabel }) }) }))] })] }));
}
