...
export function FmMobileDevToolbar() {
    ...
    const { isMainDrawerOpen, openMainDrawer, closeMainDrawer, activeTool, openTool, closeTool, badges, totalBadges, } = useMobileDevTools();
    ...
    const handleMainDrawerChange = (open) => {
        if (open) {
            openMainDrawer();
        }
        else {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(FmMobileDevFAB, { onClick: openMainDrawer, badgeCount: totalBadges }), _jsx(FmMobileDevDrawer, { open: isMainDrawerOpen, onOpenChange: handleMainDrawerChange, onToolSelect: openTool, badges: badges }), _jsx(FmMobileDevToolContent, { toolId: activeTool, open: activeTool !== null, onClose: handleToolDrawerClose, isAdmin: isAdmin() })] }));
}
