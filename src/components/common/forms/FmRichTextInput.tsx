...
interface FmRichTextInputProps {
...
const ToolbarButton = ({
  ...
  <button
    ...
    className={cn(
      ...
      isActive ? 'bg-fm-gold/30 text-fm-gold' : 'text-muted-foreground'
    )}
  >
    {children}
  </button>
...
const EditorToolbar = ({ editor, disabled = false }: EditorToolbarProps) => {
  ...
  return (
    <div className='flex flex-wrap items-center gap-0.5 px-4 py-2 bg-black/10'>
      {/* Text formatting */}
      <ToolbarButton
 ...
 * Styled to match FmCommonTextField with bottom border thickening on focus
 */
export const FmRichTextInput = ({
  label,
  required = false,
  description,
  error,
  ...
  // Handle click on container to focus editor
  ...
  return (
    <div className={cn('space-y-1', containerClassName)}>
      {/* Main input container - styled like FmCommonTextField */}
      <div
        ref={containerRef}
        onClick={handleContainerClick}
        className={cn(
          'rounded-none bg-background overflow-hidden cursor-text',
      ...
      {/* Label and description below input */}
      {(label || description) && (
        <div>
          {label && (
            <Label
      ...
      {/* TipTap placeholder and syntax highlighting styles */}
      <style>{`
...
FmRichTextInput.displayName = 'FmRichTextInput';
