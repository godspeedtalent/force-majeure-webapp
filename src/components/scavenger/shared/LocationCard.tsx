...
export const LocationCard = ({
  ...
  return (
    <Card className={`p-6 transition-all duration-300 ${isFullyClaimed
        ? 'bg-muted/50 border-muted'
        : 'bg-card hover:shadow-gold border-border'
      }`}>
      <div className="flex items-start gap-4 mb-4">
        <div className={`p-3 rounded-lg ${isFullyClaimed
            ? 'bg-muted'
            : 'bg-gradient-gold'
          }`}>
          <MapPin className={`w-6 h-6 ${isFullyClaimed ? 'text-muted-foreground' : 'text-primary-foreground'
            }`} />
        </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between text-sm">
          <span className={`font-semibold ${isFullyClaimed
