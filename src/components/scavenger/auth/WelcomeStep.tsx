      ...
      action={
        <>
          <p className="text-lg text-white mb-6">
            You've been added to the <span className="text-fm-gold font-semibold">LF SYSTEM</span> guestlist. See you there.
          </p>
          <p className="text-sm text-muted-foreground">
            Your name will be listed as <span className="text-white font-medium">{userDisplayName}</span> in the guestlist.
            If this is incorrect, please reach out to <span className="text-fm-gold">@force.majeure.events</span> on Instagram to correct it.
          </p>
        </>
      }
    />
...
export function CheckpointClaimPanel({ locationName, onClaimClick, isLoading }: CheckpointClaimPanelProps) {
  return (
    <MessagePanel
      title=""
      description=""
      action={
        <>
          <h1 className="font-display text-5xl md:text-6xl mb-4">
            Welcome to the <span className="text-fm-gold">{locationName}</span> Checkpoint
          </h1>
          <p className="text-base text-muted-foreground">
            As promised, let's get your name on that guestlist for LF SYSTEM at Kingdom, Sat Oct 18th.
          </p>
          <DecorativeDivider />
          <Button
            size="lg"
...
export function CheckpointWelcomePanel({ locationName, onJoinClick, onSignInClick }: CheckpointWelcomePanelProps) {
  return (
    <MessagePanel
      title=""
      description=""
      action={
        <>
          ...
          <p className="text-base text-muted-foreground mb-4">
            Sign up for an account with us, and we'll get you on the guestlist for LF SYSTEM at Kingdom on Saturday, October 18th.
          </p>
          <DecorativeDivider />
          <Button
            size="lg"
...
export function NoCheckpointPanel({ onJoinClick, onSignInClick }: NoCheckpointPanelProps) {
  return (
    <MessagePanel
      ...
      action={
        <>
          ...
          <p className="text-base text-muted-foreground">
            You'll need to register with us to join the guestlist once you find a checklist. You can take care of that now if you'd like.
          </p>
          <Button
            size="lg"
            className="w-full max-w-xs mx-auto bg-gradient-gold hover:opacity-90 font-semibold text-black transition-all duration-300 hover:scale-105 hover:shadow-[0_0_20px_rgba(212,175,55,0.4)]"
...
export function WelcomeStep({
  ...
  return <NoCheckpointPanel onJoinClick={onJoinClick} onSignInClick={onSignInClick} />;
}