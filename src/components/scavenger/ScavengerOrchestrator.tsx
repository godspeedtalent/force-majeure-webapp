...
import { useClaimReward, useScavengerLocations, useUserClaims } from '@/hooks/useScavenger';
...
interface ScavengerOrchestratorProps {
    children: (state: ScavengerState, actions: ScavengerActions) => React.ReactNode;
    initialLocationId?: string | null;
}

export function ScavengerOrchestrator({ children, initialLocationId = null }: ScavengerOrchestratorProps) {
    ...
    const [currentStep, setCurrentStep] = useState<'welcome' | 'unauthenticated-wizard'>('welcome');
    ...
    const actions: ScavengerActions = {
        ...
        claimCheckpoint: async () => {
            ...
            try {
                ...
                toast({
                    title: "Error",
                    description: error.message || "Failed to claim checkpoint. Please try again.",
                    variant: "destructive",
                });
            } finally {
