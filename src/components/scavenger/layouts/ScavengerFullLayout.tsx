...
import { ImageAnchor } from '@/types/imageAnchor';
...
export function ScavengerFullLayout({
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (parallaxRef.current && window.innerWidth < 1024) { // Only on mobile/tablet
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1">
                      {userClaims.map((claim) => (
                        <div key={claim.id} className="flex items-center justify-between bg-primary-foreground/10 rounded-lg p-2">
