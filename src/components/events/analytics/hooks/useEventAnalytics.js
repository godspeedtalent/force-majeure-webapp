...
export const useEventAnalytics = (eventId, dateRange) => {
    return useQuery({
        ...
        `)
            ...
            const uniqueVisitors = new Set(views?.map(v => v.viewer_id || v.session_id).filter(Boolean)).size;
            ...
            completedOrders.forEach(order => {
                ...
                const orderTickets = order.items?.reduce((sum, item) => sum + item.quantity, 0) || 0;
