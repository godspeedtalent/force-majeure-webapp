...
export function TicketGroupManager({ groups, onChange }: TicketGroupManagerProps) {
  ...
  const totalTickets = groups.reduce((sum, group) => 
    sum + group.tiers.reduce((tierSum, tier) => tierSum + tier.total_tickets, 0), 0
  );
  const totalRevenue = groups.reduce((sum, group) => 
    sum + group.tiers.reduce((tierSum, tier) => tierSum + (tier.price_cents * tier.total_tickets), 0), 0
  );
  ...
  const updateTier = (groupIndex: number, tierIndex: number, updates: Partial<TicketTier>) => {
    const newGroups = [...groups];
      ...
      <div className="grid grid-cols-4 gap-4">
          ...
          <CardContent className="pt-6">
            <div className="text-center">
              <div className="h-8 w-8 mx-auto mb-2 rounded-full bg-fm-gold/20 flex items-center justify-center">
                <span className="text-lg font-bold text-fm-gold">{totalGroups}</span>
              </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              <div className="h-8 w-8 mx-auto mb-2 rounded-full bg-fm-gold/20 flex items-center justify-center">
                <span className="text-lg font-bold text-fm-gold">{totalTiers}</span>
              </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <div className={cn('w-3 h-3 rounded-full', colorConfig.value)} />
                        <h5 className="font-semibold">{group.name || `Group ${index + 1}`}</h5>
                        {group.description && (
                      ...
                      <div className="flex items-center gap-6 text-sm text-muted-foreground ml-6">
                        <span>{group.tiers.length} tier{group.tiers.length !== 1 && 's'}</span>
                        <span>•</span>
                        <span>{groupTickets.toLocaleString()} tickets</span>
                        <span>•</span>
                        <span className="text-fm-gold font-semibold">{formatPrice(groupRevenue)}</span>
                      </div>
                    </div>
              ...
              <div className="flex items-start gap-3 flex-1">
                ...
                <div className="flex-1 space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <Select
                      value={group.color}
                      onValueChange={(value) => updateGroup(groupIndex, { color: value })}
                    >
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    ...
                    <Badge variant="outline" className="text-fm-gold border-fm-gold/50">
                  ...
                  <div className="flex-1 space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <Input
                          value={tier.name}
                          onChange={(e) => updateTier(groupIndex, tierIndex, { name: e.target.value })}
                    ...
                    <FmCommonTextField
                      ...
                      onChange={(e) => updateTier(groupIndex, tierIndex, { description: e.target.value })}
                    ...
                    <div className="flex items-center space-x-2 p-3 bg-muted/30 rounded-md">
                      <Switch
                        ...
                        onCheckedChange={(checked) =>
                          updateTier(groupIndex, tierIndex, { hide_until_previous_sold_out: checked })
                        }
                      />
                      <div className="flex-1">
                        <Label htmlFor={`hide-${groupIndex}-${tierIndex}`} className="cursor-pointer text-xs">
                          Hide until previous tier sold out
                        </Label>
                        {tierIndex === 0 && tier.hide_until_previous_sold_out && (
                          <div className="flex items-center gap-1 text-xs text-amber-500 mt-1">
                            <AlertCircle className="h-3 w-3" />
                            <span>First tier in group - will be visible immediately</span>
                          </div>
                        )}
                      </div>
                    </div>
  ...
  return (
    <div className="flex gap-6 min-h-[600px]">
              ...
              <div className="space-y-1">
                {groups.map((group, index) => {
                  ...
                  return (
                    ...
                    >
                      <div className={cn('w-3 h-3 rounded-full flex-shrink-0', colorConfig.value)} />
      ...
      <div className="flex-1 min-w-0">
        {activeView === 'overview' ? renderOverview() : renderGroupDetail(activeView)}
      </div>
    </div>
  );
}
