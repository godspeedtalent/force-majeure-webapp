...
export class TicketPDFGenerator {
     ...
     * @private
     */
    static async addTicketPageContent(doc, ticketData, _options = {}) {
        // Set up dimensions
        ...
        doc.setFontSize(18);
        doc.setTextColor(goldColor);
        const eventNameLines = doc.splitTextToSize(ticketData.eventName, pageWidth - margin * 2);
        ...
        doc.text('This ticket is valid for one admission only. Do not share or duplicate.', pageWidth / 2, footerY, { align: 'center' });
        doc.text(`Ticket ID: ${ticketData.ticketId}`, pageWidth / 2, footerY + 4, { align: 'center' });
    }
    /**
     ...
     */
    static async generateSingleTicket(ticketData, options = {}) {
        try {
     ...
     * @returns Base64 encoded PDF string
     */
    static async generateMultipleTickets(tickets, options = {}) {
        try {
            if (tickets.length === 0) {
    ...
    static async generateOrderTickets(orderData) {
        try {
            ...
            const ticketData = orderData.tickets.map(ticket => ({
                ticketId: ticket.id,
