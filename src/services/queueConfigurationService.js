 ...
 */
export async function fetchQueueConfiguration(eventId) {
    try {
 ...
 * Admin-only function.
 */
export async function upsertQueueConfiguration(config) {
    try {
        const { data, error } = await supabase
            .from('queue_configurations')
            .upsert({
            event_id: config.event_id,
            max_concurrent_users: config.max_concurrent_users ?? DEFAULT_QUEUE_CONFIG.max_concurrent_users,
            checkout_timeout_minutes: config.checkout_timeout_minutes ??
 ...
 * Admin-only function.
 */
export async function deleteQueueConfiguration(eventId) {
    try {
