...
export class TicketPDFService {
     ...
     */
    static async generateTicketPDF(data, options = {}) {
        try {
            // Validate options
        ...
        `)
            ...
            const orderData = {
                ...
                tickets: orderAny.tickets.map((ticket) => ({
                    id: ticket.id,
            ...
            // Generate PDF
            const pdfBase64 = await TicketPDFGenerator.generateOrderTickets(orderData);
     ...
     */
    static async generateIndividualTicketPDFs(data, options = {}) {
        try {
            // Validate options
            if (!this.validateOptions(options)) {
                pdfLogger.error('Invalid PDF options provided', { options });
        ...
        `)
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const orderAny = order;
            const eventDate = new Date(orderAny.events?.start_time || new Date().toISOString());
            // Generate individual PDFs for each ticket
            const pdfPromises = orderAny.tickets.map((ticket) => TicketPDFGenerator.generateSingleTicket({
                ticketId: ticket.id,
