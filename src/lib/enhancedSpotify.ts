...
interface SpotifyAuthService {
  ...
  exchangeCodeForTokens: (code: string, redirectUri: string, userId: string) => Promise<{ success: boolean; error?: any }>;
  refreshUserToken: (userId: string) => Promise<{ accessToken?: string; error?: any }>;
  getUserAccessToken: (userId: string) => Promise<{ accessToken?: string; error?: any }>;
}
...
class EnhancedSpotifyService implements SpotifyAuthService {
  ...
  async exchangeCodeForTokens(code: string, redirectUri: string, userId: string): Promise<{ success: boolean; error?: any }> {
    try {
      const { data, error } = await supabase.functions.invoke('spotify-auth', {
        body: {
  ...
  async refreshUserToken(userId: string): Promise<{ accessToken?: string; error?: any }> {
    try {
      const { data, error } = await supabase.functions.invoke('spotify-auth', {
        body: {
  ...
  async getUserAccessToken(userId: string): Promise<{ accessToken?: string; error?: any }> {
    try {
  ...
  async initializeWebPlayback(accessToken: string): Promise<Spotify.Player | null> {
  ...
  private createPlayer(accessToken: string, resolve: (player: Spotify.Player | null) => void) {
  ...
  async playTrack(player: Spotify.Player, trackUri: string, accessToken: string): Promise<boolean> {
    try {
      // Get the device ID
      ...
      const response = await fetch(`https://api.spotify.com/v1/me/player/play?device_id=${state.device_id}`, {
        method: 'PUT',
        body: JSON.stringify({
          uris: [trackUri]
        }),
