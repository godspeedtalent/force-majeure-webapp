...
export const useStripePayment = () => {
     ...
     */
    const processPayment = useCallback(async (amount, saveCard = false, savedCardId) => {
        if (!stripe || !elements) {
        ...
        try {
            ...
            const paymentIntent = await stripeService.createPaymentIntent(amount, customerId, paymentMethodId);
            // Confirm payment
            const { error: confirmError } = await stripe.confirmCardPayment(paymentIntent.clientSecret, savedCardId ? undefined : { payment_method: paymentMethodId });
            if (confirmError) {
     ...
     */
    const removeSavedCard = useCallback(async (paymentMethodId) => {
        try {
            setLoading(true);
