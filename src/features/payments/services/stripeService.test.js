...
describe('stripeService', () => {
    ...
    describe('getOrCreateCustomer', () => {
        it('should create customer with email and userId', async () => {
            ...
            const result = await stripeService.getOrCreateCustomer('test@example.com', 'user-123');
        ...
        it('should return existing customer data', async () => {
            ...
            const result = await stripeService.getOrCreateCustomer('existing@example.com', 'user-456');
        ...
        it('should throw on edge function error', async () => {
            ...
            await expect(stripeService.getOrCreateCustomer('test@example.com', 'user-123')).rejects.toThrow('Customer creation failed');
        });
        it('should throw with default message when error has no message', async () => {
            ...
            await expect(stripeService.getOrCreateCustomer('test@example.com', 'user-123')).rejects.toThrow('Failed to get Stripe customer');
        });
    });
    describe('createPaymentIntent', () => {
        ...
        it('should throw on error with correct message', async () => {
            ...
            await expect(stripeService.createPaymentIntent(50.0, 'cus_123')).rejects.toThrow('Insufficient funds');
        });
        it('should throw with default message when error has no message', async () => {
            vi.mocked(supabase.functions.invoke).mockResolvedValue({
                data: null,
            ...
            await expect(stripeService.createPaymentIntent(50.0, 'cus_123')).rejects.toThrow('Failed to create payment intent');
        });
    });
    describe('listPaymentMethods', () => {
        ...
        it('should throw on error', async () => {
            ...
            await expect(stripeService.listPaymentMethods('cus_invalid')).rejects.toThrow('Customer not found');
        });
    ...
    describe('attachPaymentMethod', () => {
        ...
        it('should not throw on success', async () => {
            ...
            await expect(stripeService.attachPaymentMethod('pm_123', 'cus_456')).resolves.toBeUndefined();
        });
        it('should throw on error', async () => {
            vi.mocked(supabase.functions.invoke).mockResolvedValue({
                data: null,
            ...
            await expect(stripeService.attachPaymentMethod('pm_123', 'cus_456')).rejects.toThrow('Payment method already attached');
        });
        it('should throw with default message when error has no message', async () => {
            vi.mocked(supabase.functions.invoke).mockResolvedValue({
                data: null,
            ...
            await expect(stripeService.attachPaymentMethod('pm_123', 'cus_456')).rejects.toThrow('Failed to attach payment method');
        });
    });
    describe('detachPaymentMethod', () => {
        ...
        it('should not throw on success', async () => {
            ...
            await expect(stripeService.detachPaymentMethod('pm_123')).resolves.toBeUndefined();
        });
        it('should throw on error', async () => {
            vi.mocked(supabase.functions.invoke).mockResolvedValue({
                data: null,
            ...
            await expect(stripeService.detachPaymentMethod('pm_invalid')).rejects.toThrow('Payment method not found');
        });
        it('should throw with default message when error has no message', async () => {
            vi.mocked(supabase.functions.invoke).mockResolvedValue({
                data: null,
            ...
            await expect(stripeService.detachPaymentMethod('pm_123')).rejects.toThrow('Failed to detach payment method');
        });
    });
});
