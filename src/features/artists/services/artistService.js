 ...
 */
export async function getArtistWithGenres(artistId) {
    try {
 ...
 * Add genre to artist
 */
export async function addGenreToArtist(artistId, genreId, isPrimary = false) {
    try {
        logApi({
 ...
 */
export async function removeGenreFromArtist(artistId, genreId) {
    try {
        logApi({
 ...
 */
export async function setPrimaryGenre(artistId, genreId) {
    try {
        logApi({
 ...
 */
export async function updateArtistGenres(artistId, genreSelections) {
    try {
        logApi({
 ...
 */
export async function checkArtistExistsByName(name) {
    try {
 ...
 */
export async function checkArtistExistsBySpotifyId(spotifyId) {
    try {
 ...
 */
export async function checkArtistExistsBySoundcloudId(soundcloudId) {
    try {
 ...
 */
export async function getArtistsByGenre(genreId, includeSubgenres = true, limit = 50) {
    try {
        logApi({
        ...
        const artistIds = [...new Set(data.map((row) => row.artist_id))].slice(0, limit);
        const artists = await Promise.all(artistIds.map(id => getArtistWithGenres(id)));
 ...
 */
export async function searchArtists(query, genreFilter, limit = 20) {
    try {
        ...
        if (genreFilter) {
            const artistGenres = await Promise.all(artists.map(async (artist) => ({
                artist,
                genres: await getArtistGenres(artist.id),
            })));
            artists = artistGenres
                .filter(({ genres }) => genres.some(g => g.genre.name.toLowerCase().includes(genreFilter.toLowerCase())))
                .map(({ artist }) => artist);
        }
        return artists;
    }
