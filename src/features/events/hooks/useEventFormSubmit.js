...
export function useEventFormSubmit(options) {
    ...
    const submitEvent = async (state) => {
        ...
        try {
            ...
            const eventTitle = state.title || (headliner && venue
            ...
            const userMessage = error?.hint || error?.details || err.message;
...
// Helper functions
async function createTicketTiers(eventId, ticketTiers) {
...
async function updateTicketTiers(eventId, ticketTiers) {
    ...
    if (existingTiers && existingTiers.length > 0) {
        ...
        const tiersWithOrders = new Set(ordersData?.map(item => item.ticket_tier_id) || []);
