 ...
 */
export function exportToCSV(data, columns, selectedColumnKeys, filename = 'export') {
    const separator = ',';
    exportDelimited(data, columns, selectedColumnKeys, separator, filename, 'csv');
}
/**
 * Export data to TSV format
 */
export function exportToTSV(data, columns, selectedColumnKeys, filename = 'export') {
    const separator = '\t';
    exportDelimited(data, columns, selectedColumnKeys, separator, filename, 'tsv');
}
/**
 * Export data to JSON format
 */
export function exportToJSON(data, columns, selectedColumnKeys, filename = 'export') {
 ...
 */
function exportDelimited(data, columns, selectedColumnKeys, separator, filename, extension) {
    const selectedColumns = columns.filter(col => selectedColumnKeys.includes(col.key));
    // Create header row
    const headers = selectedColumns.map(col => escapeDelimited(col.label, separator));
    ...
    // Download file
    const mimeType = extension === 'csv' ? 'text/csv' : 'text/tab-separated-values';
 ...
 */
export function exportData(data, columns, selectedColumnKeys, format, filename = 'export') {
    switch (format) {
        case 'csv':
