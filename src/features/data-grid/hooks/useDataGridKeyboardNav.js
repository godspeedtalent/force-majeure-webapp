 ...
 */
export function useDataGridKeyboardNav({ rows, columns, isEditing, editingCell, onStartEditing, onStopEditing, onNavigate, }) {
    const focusedCellRef = useRef(null);
    /**
     * Get the next/previous editable column
     */
    const getAdjacentColumn = (currentColumnKey, direction) => {
        const editableColumns = columns.filter(col => col.editable);
        const currentIndex = editableColumns.findIndex(col => col.key === currentColumnKey);
        if (currentIndex === -1)
            return null;
    ...
    const handleTableKeyDown = useCallback((e) => {
        ...
        if (newRowIndex !== rowIndex || newColumnKey !== columnKey) {
            ...
            const cellElement = document.querySelector(`[data-row="${newRowIndex}"][data-column="${newColumnKey}"]`);
            cellElement?.focus();
        }
     ...
     */
    const getFocusableCellProps = useCallback((rowIndex, columnKey) => {
        const column = columns.find(c => c.key === columnKey);
        const isCurrentlyEditing = editingCell?.rowIndex === rowIndex &&
