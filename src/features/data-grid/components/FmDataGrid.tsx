...
export function FmDataGrid<T extends Record<string, any>>({
  ...
  const handleCellSave = async (
    ...
    const loadingToast = toast.loading(`Updating ${displayName}...`);
  ...
  const handleSaveNewRow = async () => {
    ...
    const loadingToast = toast.loading(`Creating ${resourceName}...`);
  ...
  const handleBatchDelete = async () => {
    ...
    const loadingToast = toast.loading(`Deleting ${selectedRowsData.length} ${resourceName}${selectedRowsData.length !== 1 ? 's' : ''}...`);
  ...
  const handleBulkEdit = async (updates: Partial<T>) => {
    ...
    const selectedRowsData = filteredData.filter((_, idx) => selection.selectedRows.has(idx));
    const loadingToast = toast.loading(`Updating ${selectedRowsData.length} row${selectedRowsData.length !== 1 ? 's' : ''}...`);
