...
const getActivityLogsTable = () => (supabase as any).from('activity_logs');
...
export async function getActivitySummary(
  ...
  try {
    ...
    const categoryCounts = (data || []).reduce(
      (acc: Record<ActivityCategory, number>, item: { category: string }) => {
...
export async function triggerArchive(
  ...
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data, error } = await (supabase as any).rpc('archive_old_activity_logs', {
