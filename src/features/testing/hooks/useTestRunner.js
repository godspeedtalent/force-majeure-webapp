...
function testRunnerReducer(state, action) {
    switch (action.type) {
        case 'START':
            return {
                ...state,
...
export function useTestRunner(options) {
    ...
    const runSuite = useCallback(async (suite) => {
        try {
            ...
            const results = await runnerRef.current.runTests(suite.testCases, (activeThreads, results) => {
